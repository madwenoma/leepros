...
public abstract class AbstractQueuedSynchronizer extends AbstractOwnableSynchronizer
    ...
    private static Logger logger = LoggerFactory.getLogger(AbstractQueuedSynchronizer.class);
    ...
    /**
     * 头结点，懒加载，只被setHead方法修改，如果head存在，watiStatus保证不为Canclled
     */
    ...
    private void setHead(Node node) {
        head = node;
        node.thread = null;//thread设置为null 成为0号节点，是拿到了锁的？
        node.prev = null;
    }
    ...
    private void debug(String methodName, Node s) {
        if (s == null) {
            logger.info(methodName + "-" + Thread.currentThread().getId() + " node is null");
        } else {
            logger.info(methodName + "-" + Thread.currentThread().getId() + " node :threadId-{},waitStatus-{}", s.thread.getId(), s.waitStatus);
    ...
    private final boolean parkAndCheckInterrupt() {
        logger.info("parkAndCheckInterrupt begin to part thread -{}", Thread.currentThread().getId());
    ...
    //acquireInterruptibly由于没有获取到同步状态而发生阻塞之后，如果有别的线程中断了这个线程，
    //则acquireInterruptibly方法会抛出InterruptedException异常并返回
    ...
    public final boolean release(int arg) {
        if (tryRelease(arg)) {//stat修改成功
            Node h = head;//头结点就是拿到锁的节点
            logger.info("release -{} ", Thread.currentThread().getId());
...
//    private static final Unsafe unsafe = Unsafe.getUnsafe();
    ...
    private final boolean compareAndSetHead(Node update) {
        logger.info("compareAndSetHead 将Node{}设置为队头", update);
    ...
    private final boolean compareAndSetTail(Node expect, Node update) {
        logger.info("compareAndSetTail{}尝试将node{}设置为队尾", Thread.currentThread().getId(), update.thread.getId());
